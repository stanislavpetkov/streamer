...
static inline void print_stream_params(AVIOContext *pb, FFServerStream *stream)
{
	...
	if (i>0) {
        ...
        avio_printf(pb, "{\"streamNo\": \"%d\", \"type\": \"%s\", \"bitrate\": \"%"PRId64"\", \"name\":\"%s\", \"params\": \"%s\"}",
...
static void compute_status(HTTPContext *c)
{
    ...
    avio_printf(pb, "Content-type: application/json\r\n");
    avio_printf(pb, "Pragma: no-cache\r\n");
    avio_printf(pb, "\r\n");

    avio_printf(pb, "{\"appName\": \"%s\",", program_name);
...
start = 1;
        ...
        avio_printf(pb, "{\"streamName\": \"%s\", \"streamUrl\": \"%s\", \"streamConnCount\": \"%d\", \"streamBytesServed\": \"%ju\"",
                    sfilename, stream->filename, stream->conns_served, stream->bytes_served);
        ...
        case STREAM_TYPE_LIVE: {
            ...
            avio_printf(pb, ", \"formatName\": \"%s\", \"streamBandwidth\": \"%d\", \"videoBitRate\": \"%d\", \"videoCodecName\": \"%s\", \"videoCodecNameEx\": \"%s\", \"audioBitRate\": \"%d\", \"audioCodecName\": \"%s\", \"audioCodecNameEx\": \"%s\"",
            ...
            if (stream->feed)
                avio_printf(pb, ", \"feedFileName\": \"%s\"}", stream->feed->filename);
            else
                avio_printf(pb, ", \"feedFileName\": \"Unknown\"}");
        }
            break;
        default:
		avio_printf(pb, "}");
...
start = 1;
        ...
        avio_printf(pb, "{\"feedName\": \"%s\"", stream->filename);
        if (stream->pid) {
            avio_printf(pb, ", \"runningPid\": \"%"PRId64"\"", (int64_t) stream->pid);
                ...
                snprintf(ps_cmd, sizeof(ps_cmd),
                 ...
                 if (pid_stat) {
                     ...
                     if (fscanf(pid_stat, "%9s %63s", cpuperc, cpuused) == 2) {
                         avio_printf(pb, ", \"cpuUsage\": \"%s%%\"", cpuperc);
                     }
                     fclose(pid_stat);
                 }
            }
...
avio_printf(pb, "]");
    /* connection status */
    avio_printf(pb, ", \"connectionStatus\": {");

    avio_printf(pb, "\"connectionsCount\": \"%d\", \"maxConnectionsCount\": \"%d\"",
                nb_connections, config.nb_max_connections);

    avio_printf(pb, ", \"bandwidthInUse\": \"%"PRIu64"\", \"maxBandWidth\": \"%"PRIu64"\"",
                current_bandwidth, config.max_bandwidth);

    avio_printf(pb, ", \"connections\": ["); //<table>
...
start = 1;
        ...
        avio_printf(pb, "{\"connNo\": \"%d\", \"file\": \"%s\", \"type\": \"%s\", \"IP\": \"%s\", \"proto\": \"%s\", \"state\": \"%s\", \"targetDataRate\": \"%d\", \"actualDataRate\": \"%d\", \"bytesTransfered\":\"%"PRIu64"\"}",
                    i, c1->stream ? c1->stream->filename : "",
                    c1->state == HTTPSTATE_RECEIVE_DATA ? "(input)" : "", p,
                    c1->protocol, http_state[c1->state], bitrate, compute_datarate(&c1->datarate, c1->data_count) * 8, c1->data_count);
        c1 = c1->next;
    }
...
//    avio_printf(pb, ", \"generatedTime\": \"%A, %B %d %Y\"", tmp);
    avio_printf(pb, "}");
...
static void compute_status_OLD(HTTPContext *c)
{
    HTTPContext *c1;
    FFServerStream *stream;
    char *p;
